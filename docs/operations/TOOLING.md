...
| `scripts/investigate_event.py` | **Is this one event's call open?** Walks the event's own site through the fetch ladder and reports verbatim evidence either way. Ours, not upstream's - once we hold a domain, re-reading it is verification. | **Two passes, run BLIND to each other, then compared.** Regex triages which pages deserve a model call and offers a candidate; the model answers the open question independently and its answer must be a literal substring of the page. Showing the model the regex answer would buy agreement bias, not confirmation. A scorecard records which pass answered; across six real sites it read agreed 7, disagreed 3, **model-only 0**, regex-only 5 - so triage is not dropping pages, and the regex candidate is a sometimes-right second opinion worth keeping. Model-only going non-zero is the number to watch. **It follows the site's own menu links** rather than guessing paths, because guessing reached 3 of 20 on embedded world and 1 of 20 on PCIM; guessed paths remain the fallback for JavaScript navs with no hrefs to read. Two failures are reported as themselves and not as "no findings": a **dead site** (nothing reachable - the row needs a new URL) and **catch-all routing** (every address returns one page - the sections are behind JavaScript). `--no-llm` for the free-but-noisy pass alone. It gathers evidence and does not decide: 2.1 holds, an absent call-for-speakers page is not proof the call is shut. |
